...
{
    class CviciciPredmetu : View
    {

        public CviciciPredmetu(Dictionary<string, Object> context)
            : base(context)
        {
        }
        ...
        public override void Render()
        {
            ...
            do // volba editace
            {
                Console.Clear();
                Console.WriteLine();
                Console.WriteLine("\tSPRÁVA CVIČÍCÍCH PŘEDMĚTU [{0}]", predmet.Zkratka);
                Console.WriteLine("\r\n\tZvolte další postup:");
                Console.WriteLine();
                Console.WriteLine("\t\t1: Výpis cvičících předmětu");
                Console.WriteLine("\t\t2: Plnění předmětu cvičícími");
            ...
            switch (volba)
            {
                case 1: // Výpis vyučujících předmětu + odstaňování
                    do
                    {
                        ...
                        var serazeniVyucujici = from pair in predmet.Cvicici
                        ...
                        if(count == 0)
                        {
                            Console.WriteLine("\r\n\t\tPředmět zatím neobsahuje žádné cvičící.");
                            Console.Write("\r\n\t\tStiskněte libovolnou klávesu pro návrat. ");
                            Console.ReadKey();
                            Request("Predmet", "CviciciPredmetu", predmet);
                        }
                            ...
                            if (itrBuffer == bufferSize || (count < bufferSize && itrBuffer == count) || i == count - 1)
                            {
                                do
                                {
                                    ...
                                    for (int j = 0; j < actualId.Count; j++)
                                    {
                                        if (odd)
                                        {
                                            if (MaCviciciRozvrhovouAkciPredmetu(vyucujici[actualId[j]], predmet))
                                            {
                                    ...
                                    do
                                    {
                                        ...
                                        {
                                            Console.Write("\r\n\tZadejte ID cvičícího, kterého chcete odebrat, nebo enter posun/ukončení: ");
                                            input = Console.ReadLine();
                                            ok = int.TryParse(input, out item);
                                            if (ok) ok = actualId.Contains(item);
                                            if (!ok) ok = input == String.Empty;
                                            if (input != String.Empty && MaCviciciRozvrhovouAkciPredmetu(vyucujici[item], predmet))
                                            {
                                                ok = false;
                                            }
                                        }
                        ...
                        for (i = 0, itrBuffer = 0; i < count; i++)
                        {
                            ...
                            if (itrBuffer == bufferSize || (count < bufferSize && itrBuffer == count) || i == count - 1)
                            {
                                do
                                {
                                    ...
                                    do
                                    {
                                        Console.Write("\r\n\tZadejte id cvičícího, kterého chcete přidat, nebo enter posun/ukončení: ");
                                        input = Console.ReadLine();
                                        ok = int.TryParse(input, out item);
                                        if (ok) ok = actualId.Contains(item);
                                        if (!ok) ok = input == String.Empty;
        ...
        private void printHeader(Models.Predmet predmet)
        {
            Console.Clear();
            Console.WriteLine();
            Console.WriteLine("\t\tSpráva cvičících předmětu: {0}", predmet);
        ...
        private bool MaCviciciRozvrhovouAkciPredmetu(Models.Vyucujici ucitel, Models.Predmet predmet)
        {
            foreach (KeyValuePair<Dny, Dictionary<int, Models.RozvrhovaAkce>> rozvrhDne in ucitel.Rozvrh)
            {
                ...
                {
                    if (akce.Value.TypVyuky == TypyVyuky.Cvičení && akce.Value.Predmet == predmet)
                    {
                        return true;
                    }
                }
